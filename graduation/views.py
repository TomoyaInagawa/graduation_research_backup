...
class Guide_MylistView(generic.ListView):
    model = GuideCourse
    template_name = "guide/guide_list.html"

    def get_queryset(self):
        return GuideCourse.objects.filter(author = self.request.user)
    ...
    def post(self, request):
        ...
        if sort == "0":
            if make == "0":
                result = GuideCourse.objects.filter(author = self.user).order_by('addDateTime')
            elif make == "1":
                result = GuideCourse.objects.filter(author = self.user).order_by('addDateTime').reverse()
        elif sort == "1":
            if name == "0":
                result = GuideCourse.objects.filter(author = self.user).order_by('title')
            elif name == "1":
                result = GuideCourse.objects.filter(author = self.user).order_by('title').reverse()
        params = {
            'guidecourse_list' : result,
        }
        return render(request, 'guide/guide_mylist.html', params)
